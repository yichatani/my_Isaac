...
# ---- 新增：加载点云 ----
def load_episode_pointclouds(episode_dir):
    """加载 episode 中的所有点云"""
    pc_dir = os.path.join(episode_dir, "pointclouds")
    assert os.path.exists(pc_dir), f"Missing {pc_dir}"

    files = sorted([f for f in os.listdir(pc_dir) if f.endswith(".npy")])
    pointclouds = []

    for f in files:
        pc = np.load(os.path.join(pc_dir, f))  # (N, 6)
        pointclouds.append(pc)

    pointclouds = np.array(pointclouds, dtype=np.float32)  # (T, N, 6)
    return pointclouds
...
def process_episode(episode_dir):
    ...
    pointclouds = load_episode_pointclouds(episode_dir)  # 新增

    T = eef_poses.shape[0]
    assert images.shape[0] == T, "Pose/Image length mismatch"
    assert pointclouds.shape[0] == T, "Pose/PointCloud length mismatch"  # 新增
    ...
    # ---- pointclouds ----
    pointclouds = pointclouds[:-1]  # 新增

    return states, actions, images, pointclouds
...
def scan_episodes(episodes_root):
    ...
    for ep_name in episode_names:
        ...
        if pointcloud_shape is None:
            pc_dir = os.path.join(ep_dir, "pointclouds")
            files = sorted([f for f in os.listdir(pc_dir) if f.endswith(".npy")])
            if len(files) > 0:
                pc = np.load(os.path.join(pc_dir, files[0]))  # (N, 6)
                pointcloud_shape = pc.shape  # (N, 6)
        
        del eef_poses  # 释放内存
    
    return episode_names, traj_lengths, image_shape, pointcloud_shape
...
def convert_episodes_to_npz(episodes_root, save_dir):
    ...
    episode_names, traj_lengths, image_shape, pointcloud_shape = scan_episodes(episodes_root)
    
    total_len = sum(traj_lengths)
    print(f"\nFound {len(episode_names)} episodes")
    print(f"Total trajectory length: {total_len}")
    print(f"Image shape: {image_shape}")
    print(f"PointCloud shape: {pointcloud_shape}")  # 新增
    ...
    # 新增：点云 mmap
    pointclouds_mmap = np.lib.format.open_memmap(
        os.path.join(save_dir, 'pointclouds.npy'),
        mode='w+',
        dtype=np.float32,
        shape=(total_len,) + pointcloud_shape
    )
    ...
    for ep_name in episode_names:
        ...
        states, actions, images, pointclouds = process_episode(ep_dir)  # 修改
        traj_len = len(states)
        ...
        pointclouds_mmap[idx:idx+traj_len] = pointclouds  # 新增
        
        idx += traj_len
        
        # 释放内存
        del states, actions, images, pointclouds  # 修改
    
    # 刷新到磁盘
    states_mmap.flush()
    actions_mmap.flush()
    images_mmap.flush()
    pointclouds_mmap.flush()  # 新增
    ...
    np.savez_compressed(
        ...
        images=images_mmap,
        pointclouds=pointclouds_mmap,  # 新增
        traj_lengths=np.array(traj_lengths, dtype=np.int64)
    )
    
    # 清理临时文件
    del states_mmap, actions_mmap, images_mmap, pointclouds_mmap  # 修改
    ...
    os.remove(os.path.join(save_dir, 'pointclouds.npy'))  # 新增
    ...
    print(f"  pointclouds: ({total_len}, {pointcloud_shape[0]}, {pointcloud_shape[1]})")  # 新增
