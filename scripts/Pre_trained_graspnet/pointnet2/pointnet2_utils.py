...
class GatherOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        # if torch.any(idx < 0) or torch.any(idx >= N):
        #     raise ValueError(f"indices element out of data bounds, must be within the inclusive range [0, {N-1}]")
    ...
    def symbolic(g, features, idx):
        return g.op("Gather", features, idx, axis_i=4)
...
def grouping_operation(features, idx):
    ...
    # Assuming '_ext.group_points' is an external function you're using
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        # idx = _ext.ball_query(new_xyz, xyz, self.radius, self.nsample)
...
class GroupAll(nn.Module):
    ...
    @staticmethod
    def symbolic(g, use_xyz=True, ret_grouped_xyz=False):
        
        return g.op("custom::GroupAll", use_xyz=True, ret_grouped_xyz=False)
