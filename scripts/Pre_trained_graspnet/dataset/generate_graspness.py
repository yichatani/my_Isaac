...
parser.add_argument('--graspness', default='t')
...
def generate_graspness():
    ...
    save_path_root = os.path.join(dataset_root, 'Affordance', 'graspness_avg')

    num_views, num_angles, num_depths = 300, 12, 4
    fric_coef_thresh = 0.8
    point_grasp_num = num_views * num_angles * num_depths
    for scene_id in range(77,78):
        ...
        for ann_id in range(256):
            ...
            for i, (obj_idx, trans_) in enumerate(zip(obj_list, pose_list)):
                ...
                # computer graspness
                graspness = np.sum((1.1 - np.maximum(fric_coefs, 0)) * collision.astype(int), axis=(1, 2, 3)) / point_grasp_num
