...
def process_grasp_labels(end_points):
    ...
    for i in range(batch_size):
        ...
        for obj_idx, pose in enumerate(poses):
            ...
            # print('grasp_views shape', grasp_points)
            # print('grasp shape', pose)
    ...
    batch_grasp_view_valid_mask = (batch_grasp_scores >= 0.3)
...
def process_meta_grasp_labels(end_points):
    ...
    for i in range(batch_size):
        seed_xyz = seed_xyzs[i]  # (Ns, 3)
        grasps = end_points['grasp_points_list'][i]
        # print('gen_len', end_points['grasp_points_list'].shape)
        ...
        grasp_scores_merged = []
        grasp_widths_merged = []
        for grasp_idx, pose in enumerate(grasps):
            grasp_points = end_points['grasp_points_list'][i][grasp_idx]  # (Np, 3)
            grasp_points = torch.stack([grasp_points]).to(pose.device)
            grasp_scores = end_points['grasp_scores_list'][i][grasp_idx]  # (Np, V, A, D)
            grasp_scores = torch.stack([grasp_scores]).to(pose.device)
            grasp_widths = end_points['grasp_widths_list'][i][grasp_idx]  # (Np, V, A, D)
            grasp_widths = torch.stack([grasp_widths]).to(pose.device)
            _, V, A, D = grasp_scores.size()
            num_grasp_points = grasp_points.size(0)
            # generate and transform template grasp views
            grasp_views = generate_grasp_views(V).to(pose.device)  # (V, 3)
            ...
            angles = torch.zeros(grasp_views.size(0), dtype=grasp_views.dtype, device=grasp_views.device)
            grasp_views_rot = batch_viewpoint_params_to_matrix(-grasp_views, angles)  # (V, 3, 3)
            grasp_views_rot_trans = torch.matmul(torch.tensor([[0,0,0]] * 3, dtype=torch.float32).to(grasp_views.device), grasp_views_rot)  # (V, 3, 3)

            # assign views
            grasp_views_ = grasp_views.transpose(0, 1).contiguous().unsqueeze(0)
            #grasp_views_trans_ = grasp_views_trans.transpose(0, 1).contiguous().unsqueeze(0)
            grasp_views_trans_ = grasp_views.transpose(0, 1).contiguous().unsqueeze(0)
            view_inds = knn(grasp_views_trans_, grasp_views_, k=1).squeeze() - 1
            grasp_views_rot_trans = torch.index_select(grasp_views_rot_trans, 0, view_inds)  # (V, 3, 3)
            grasp_views_rot_trans = grasp_views_rot_trans.unsqueeze(0).expand(num_grasp_points, -1, -1,
                                                                            -1)  # (Np, V, 3, 3)
            ...
            # grasp_widths = torch.index_select(grasp_widths, 1, view_inds)  # (Np, V, A, D)
            # add to list
            grasp_points_merged.append(grasp_points)
            grasp_views_rot_merged.append(grasp_views_rot_trans)
            grasp_scores_merged.append(grasp_scores)
            grasp_widths_merged.append(grasp_widths)
    ...
    # process scores
    # print('before', np.where(batch_grasp_scores.detach().cpu().numpy() >0))
    label_mask = (batch_grasp_scores > 0) & (batch_grasp_widths <= GRASP_MAX_WIDTH)  # (B, Ns, V, A, D)
    batch_grasp_scores[~label_mask] = 0
    # print('after', np.where(batch_grasp_scores.detach().cpu().numpy() >0))
