...
def batch_viewpoint_params_to_matrix(batch_towards, batch_angle):
    ...
    norm_y = torch.norm(axis_y, dim=-1, keepdim=True)

    # axis_y[mask_y, 1] = 1
    mask_y = norm_y == 0  # Find where the norm is zero

    # Use torch.where to replace the y-component conditionally
    axis_y[:, 1] = torch.where(mask_y.squeeze(), torch.ones_like(axis_y[:, 1]), axis_y[:, 1])
