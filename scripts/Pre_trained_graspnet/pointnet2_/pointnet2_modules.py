...
class GroupingOperation(Function):
    ...
    def symbolic(g, features, idx):
        return g.op("custom::GroupingOperation", features, idx)
...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        fps_idxs = _ext.furthest_point_sampling(xyz, self.npoint)
        ...
        for i in range(len(self.groupers)):
            ...
            #     new_features, kernel_size=[1, new_features.size(3).item()]
            # )  # (B, mlp[-1], npoint, 1)
            kernel_size = new_features.size(3)
            if torch.is_tensor(kernel_size):
                kernel_size = kernel_size.item()
            new_features = F.max_pool2d(new_features, kernel_size=[1, kernel_size]) 
    ...
    def __init__(
        ...
        for i in range(len(radii)):
            ...
            self.groupers.append(
                ...
                QueryAndGroup(radius, nsample, use_xyz=use_xyz, sample_uniformly=sample_uniformly)
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        xyz_flipped = xyz.transpose(1, 2).contiguous()
        if inds is None:
            inds = _ext.furthest_point_sampling(xyz, self.npoint)
